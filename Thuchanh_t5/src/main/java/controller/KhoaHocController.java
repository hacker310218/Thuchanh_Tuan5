...
public class KhoaHocController {
	...
	public KhoaHoc getKhoaHoc(@PathVariable Long id) {
		try {
			return khoaHocRepository.findById(id).orElseThrow(() -> new RuntimeException("KhoaHoc not found with id: " + id));
		} catch (Exception e) {
	...
	public KhoaHoc updateKhoaHoc(@PathVariable Long id, @RequestBody KhoaHoc khoaHocData) {
		...
		try {
			khoaHoc = khoaHocRepository.findById(id)
					.orElseThrow(() -> new RuntimeException("KhoaHoc khong tim thay id: " + id));
		} catch (Exception e) {
			throw new RuntimeException(e);
		}
